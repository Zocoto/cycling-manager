...
describe("COUNTRY_EXPANSION_BATCH_01_SPONSORS", () => {
  it("couvre dix nouveaux pays avec trois maillots par sponsor", () => {
    expect(COUNTRY_EXPANSION_BATCH_01_SPONSORS).toHaveLength(10);
    expect(
      new Set(
        COUNTRY_EXPANSION_BATCH_01_SPONSORS.map(
          (sponsor) => sponsor.countryCode,
        ),
      ).size,
    ).toBe(10);
    expect(
      COUNTRY_EXPANSION_BATCH_01_SPONSORS.every(
  ...
  it("référence des identifiants et assets uniques", () => {
    const sponsorIds = COUNTRY_EXPANSION_BATCH_01_SPONSORS.map(
      (sponsor) => sponsor.id,
    );
    const jerseyIds = COUNTRY_EXPANSION_BATCH_01_SPONSORS.flatMap(
      (sponsor) => sponsor.jerseys.map((jersey) => jersey.id),
    );
    const assetPaths = COUNTRY_EXPANSION_BATCH_01_SPONSORS.flatMap(
      (sponsor) => [
        sponsor.logoPath,
  ...
  it("propose une gamme économique complète", () => {
  ...
  it("est intégré au catalogue global", () => {
    ...
    for (const sponsor of COUNTRY_EXPANSION_BATCH_01_SPONSORS) {
