...
describe("COUNTRY_EXPANSION_BATCH_04_SPONSORS", () => {
  it("couvre dix nouveaux pays avec trois maillots par sponsor", () => {
    expect(COUNTRY_EXPANSION_BATCH_04_SPONSORS).toHaveLength(10);
    expect(
      new Set(
        COUNTRY_EXPANSION_BATCH_04_SPONSORS.map(
          (sponsor) => sponsor.countryCode,
        ),
      ).size,
    ).toBe(10);
    expect(
      COUNTRY_EXPANSION_BATCH_04_SPONSORS.every(
  ...
  it("référence des identifiants et assets WebP uniques", () => {
    const sponsorIds = COUNTRY_EXPANSION_BATCH_04_SPONSORS.map(
      (sponsor) => sponsor.id,
    );
    const jerseyIds = COUNTRY_EXPANSION_BATCH_04_SPONSORS.flatMap(
      (sponsor) => sponsor.jerseys.map((jersey) => jersey.id),
    );
    const assetPaths = COUNTRY_EXPANSION_BATCH_04_SPONSORS.flatMap(
      (sponsor) => [
        sponsor.logoPath,
  ...
  it("est intégré au catalogue global", () => {
    ...
    for (const sponsor of COUNTRY_EXPANSION_BATCH_04_SPONSORS) {
