...
import { COUNTRY_EXPANSION_BATCH_12_SPONSORS } from "./country-expansion-batch-12";

describe("country expansion batch 12", () => {
  it("adds five sponsors in countries that previously had one sponsor", () => {
    expect(COUNTRY_EXPANSION_BATCH_12_SPONSORS).toHaveLength(5);
    expect(COUNTRY_EXPANSION_BATCH_12_SPONSORS.map((sponsor) => sponsor.countryCode)).toEqual([
      "AL",
      "BH",
      "BD",
      "UY",
  ...
  it("covers five distinct new industries", () => {
    expect(new Set(COUNTRY_EXPANSION_BATCH_12_SPONSORS.map((sponsor) => sponsor.sector)).size).toBe(5);
  });

  it("provides three jersey variants and complete WebP asset paths", () => {
    for (const sponsor of COUNTRY_EXPANSION_BATCH_12_SPONSORS) {
