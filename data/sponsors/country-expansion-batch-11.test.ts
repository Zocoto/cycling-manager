...
import { COUNTRY_EXPANSION_BATCH_11_SPONSORS } from "./country-expansion-batch-11";

describe("country expansion batch 11", () => {
  it("adds five conventional industries in countries that previously had one sponsor", () => {
    expect(COUNTRY_EXPANSION_BATCH_11_SPONSORS).toHaveLength(5);
    expect(COUNTRY_EXPANSION_BATCH_11_SPONSORS.map((sponsor) => sponsor.countryCode)).toEqual([
      "GH",
      "JO",
      "RO",
      "SI",
  ...
  it("covers five distinct underrepresented industries", () => {
    expect(new Set(COUNTRY_EXPANSION_BATCH_11_SPONSORS.map((sponsor) => sponsor.sector)).size).toBe(5);
  });

  it("provides three jersey variants and complete WebP asset paths", () => {
    for (const sponsor of COUNTRY_EXPANSION_BATCH_11_SPONSORS) {
