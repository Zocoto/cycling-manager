-- Permet au Directeur Sportif de licencier un membre de son staff.
-- L’indemnité couvre le salaire restant de la saison active et une saison
-- complète supplémentaire. Les échéances salariales futures sont annulées.

begin;

alter table public.staff_contracts
  add column if not exists termination_compensation numeric(14, 2),
  add column if not exists termination_season_id uuid
    references public.seasons(id) on delete set null,
  add column if not exists termination_day_number smallint;

alter table public.staff_contracts
  drop constraint if exists staff_contracts_termination_compensation_non_negative,
  drop constraint if exists staff_contracts_termination_day_range;

alter table public.staff_contracts
  add constraint staff_contracts_termination_compensation_non_negative
    check (
      termination_compensation is null
      or termination_compensation >= 0
    ),
  add constraint staff_contracts_termination_day_range
    check (
      termination_day_number is null
      or termination_day_number between 1 and 28
    );
...
comment on column public.staff_contracts.termination_compensation is
  'Indemnité payée lors du licenciement : solde de la saison active et salaire de la suivante.';
...
comment on function public.dismiss_current_team_staff(uuid) is
  'Licencie un membre du staff géré par le DS connecté et débite son indemnité de rupture.';
