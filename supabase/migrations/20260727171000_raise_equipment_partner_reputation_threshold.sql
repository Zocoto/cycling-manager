-- L'équipementier devient un objectif de progression avancé :
-- une équipe peut signer à partir de 50 points de réputation.
...
begin
  ...
  if coalesce(v_context.reputation_points, 0) < 50 then
    raise exception 'Une réputation d’au moins 50 points est nécessaire pour signer.';
  end if;
...
comment on function public.sign_equipment_partner_contract(text) is
  'Signe un contrat équipementier de deux saisons pour une équipe ayant au moins 50 points de réputation.';
