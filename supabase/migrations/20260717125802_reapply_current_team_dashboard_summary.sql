...
begin
  ...
  with current_context as (
    select
      teams.id as current_team_id,
      team_seasons.display_name as current_team_name,
      seasons.id as current_season_id,
      seasons.name as current_season_name,
      seasons.current_day_number as current_season_day_number,
      seasons.game_year as current_season_game_year
  ...
  active_contracts as (
    select
      rider_contracts.team_id as contracted_team_id,
      count(distinct rider_contracts.rider_id)::integer
        as active_rider_count
  ...
  select
    ...
    current_context.current_team_name,
    coalesce(
      active_contracts.active_rider_count,
      0
    )::integer,
    current_context.current_season_id,
    current_context.current_season_name,
    current_context.current_season_day_number
