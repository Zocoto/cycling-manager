...
alter table public.daily_reward_claims
  drop constraint daily_reward_claims_streak_range;

alter table public.daily_reward_claims
  add constraint daily_reward_claims_streak_range
  check (streak_day between 1 and 40);

create index if not exists daily_reward_claims_director_latest_idx
  on public.daily_reward_claims (sporting_director_id, claimed_at desc);
...
create table public.daily_reward_streak_states (
  ...
  last_claimed_game_day_index integer not null,
  last_claim_id uuid references public.daily_reward_claims(id) on delete set null,
  updated_at timestamptz not null default now(),
  constraint daily_reward_streak_states_cycle_day_range
    check (cycle_day between 1 and 40)
);

comment on table public.daily_reward_streak_states is
  'Progression du cycle de 40 cadeaux, persistante entre les saisons.';

alter table public.daily_reward_streak_states enable row level security;

grant all privileges on public.daily_reward_streak_states to service_role;
...
select distinct on (claim.sporting_director_id)
  claim.sporting_director_id,
  (
    case
      when claim.importance = 10 then 40
      else least(40, greatest(1, claim.streak_day))
    end
  )::smallint,
  (season.game_year * 28 + day.day_number)::integer,
  claim.id,
  claim.claimed_at
from public.daily_reward_claims as claim
join public.season_days as day on day.id = claim.season_day_id
join public.seasons as season on season.id = claim.season_id
order by
  claim.sporting_director_id,
  (season.game_year * 28 + day.day_number) desc,
...
as $$
  with normalized as (
    select mod(greatest(coalesce(p_streak_day, 1), 1) - 1, 40) + 1 as cycle_day
  )
  select case
    when cycle_day = 40 then 10
    when cycle_day = 36 then 9
    when cycle_day = 32 then 8
    when cycle_day = 28 then 7
    when cycle_day in (21, 27, 31, 35, 39) then 6
    when cycle_day in (25, 30, 34, 38) then 5
    when cycle_day in (14, 18, 22, 23, 24, 26, 29, 33, 37) then 4
    when cycle_day in (7, 11, 15, 16, 17, 19, 20) then 3
    when cycle_day in (4, 8, 9, 10, 12, 13) then 2
    else 1
  end
  from normalized;
$$;

create or replace function public.get_next_daily_reward_cycle_day(p_cycle_day integer)
returns integer
language sql
immutable
set search_path = ''
as $$
  select case
    ...
    when coalesce(p_cycle_day, 0) < 1 then 1
    else p_cycle_day + 1
  end;
$$;
...
begin
  ...
  if v_streak_state is not null then
    v_consecutive := case
      when v_streak_state.last_claimed_game_day_index in (
        v_current_game_day_index,
        v_current_game_day_index - 1
      ) then v_streak_state.cycle_day
      else 0
    end;
  end if;
  ...
  v_prospective := case
    ...
    when v_streak_state.last_claimed_game_day_index = v_current_game_day_index - 1
...
revoke all on function public.get_next_daily_reward_cycle_day(integer) from public;
grant execute on function public.get_next_daily_reward_cycle_day(integer) to authenticated, service_role;
