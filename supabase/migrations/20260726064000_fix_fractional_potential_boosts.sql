-- ============================================================
-- Corrige l'application des objets de potentiel.
-- Les payloads expriment un bonus en étoiles (par pas de 0,5),
-- tandis que riders.potential_steps stocke des demi-étoiles.
-- ============================================================
...
declare
  ...
  v_ability_code text;
  v_potential_bonus_stars numeric;
  v_potential_bonus_steps integer;
  v_rating_key text;
  v_rating_bonus integer;
  v_applied_rating_bonus integer;
...
begin
  ...
  if v_item.category = 'special_ability' then
    ...
    v_potential_bonus_stars :=
      (v_item.effect_payload ->> 'potentialBonus')::numeric;

    if coalesce(v_potential_bonus_stars, 0) <= 0
      or v_potential_bonus_stars * 2
        <> trunc(v_potential_bonus_stars * 2) then
      raise exception 'Le bonus de potentiel porté par cet objet est invalide.';
    end if;

    v_potential_bonus_steps := (v_potential_bonus_stars * 2)::integer;
    ...
    if v_current_value + v_potential_bonus_steps > 8 then
      raise exception 'Le potentiel de ce coureur est trop élevé pour appliquer entièrement cet objet.';
    end if;

    update public.riders
    set potential_steps = potential_steps + v_potential_bonus_steps
    where id = p_rider_id;
    ...
    v_applied_rating_bonus := v_rating_bonus;
    ...
    using v_applied_rating_bonus, p_rider_id;
  end if;
  ...
  values (
    ...
    v_ability_code,
    case
      when v_item.category = 'potential_boost'
        then v_potential_bonus_steps
      else null
    end,
    v_rating_key,
    case
      when v_item.category = 'rating_boost'
        then v_applied_rating_bonus
      else null
    end
  );
...
comment on column public.rider_consumable_item_applications.potential_bonus is
  'Bonus appliqué en demi-étoiles, dans la même unité que riders.potential_steps.';

comment on function public.use_current_team_inventory_item(uuid, uuid) is
  'Consomme atomiquement un objet et convertit les bonus de potentiel exprimés en étoiles vers les demi-étoiles stockées.';
