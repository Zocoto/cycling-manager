...
create or replace function public.repair_boucle_stage3_delayed_group_20260920(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
as $$
declare
  v_key constant text := 'boucle-provinces-s3-stage3-delayed-group-20260920';
  v_edition constant uuid := '716dad06-c144-47a7-afbc-68bff48f8e57';
  v_stage constant uuid := 'dc96b124-3b96-4393-ba55-c31f2915fef3';
  v_next_stage constant uuid := 'b493e2ed-4e31-401d-a17a-f839524d4a41';
  v_source_engine constant text := '2026.09-leader-recovery-priority-v30';
  v_target_engine constant text := '2026.09-delayed-group-energy-v31';
...
begin
  if p_payload is null
    ...
    or jsonb_array_length(p_payload->'stageRows') <> 177 then
  ...
  if (select count(*) from public.stages where race_edition_id = v_edition) <> 12
    or (select count(*) from public.stages
        where race_edition_id = v_edition and status = 'completed') <> 3
    or (select stage_number from public.stages where id = v_stage) <> 3
...
revoke all on function public.repair_boucle_stage3_delayed_group_20260920(jsonb)
  from public, anon, authenticated;
grant execute on function public.repair_boucle_stage3_delayed_group_20260920(jsonb)
  to service_role;
