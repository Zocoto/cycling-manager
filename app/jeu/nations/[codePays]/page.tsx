...
export default async function PublicCountryPage({
  ...
  const { riders: nationRiders, totalCount: riderCount } =
    await getNationRiderOverview(country.entity_id);
  const riderJerseysByTeamId = await getNationRiderJerseys(nationRiders);
  const riderRanking = nationRiders.map((rider) => ({
    ...rider,
    jersey: rider.teamId
      ? (riderJerseysByTeamId.get(rider.teamId) ?? FREE_AGENT_RIDER_JERSEY)
      : FREE_AGENT_RIDER_JERSEY,
  }));
    ...
    <main className="min-h-screen bg-[#EAF5F3] text-[#082A2A]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        <div className="mt-5 overflow-hidden rounded-[2rem] border border-[#315B3E]/15 bg-white shadow-[0_24px_70px_rgba(19,60,46,0.12)]">
          ...
          <NationRiderRanking riders={riderRanking} />
        </div>
