...
export async function updateSportingDirectorProfile(
  ...
  if (selectedAvatar?.outfit === EL_PRESIDENTE_AVATAR_OUTFIT_KEY) {
    const { data: presidentTrophy, error: presidentTrophyError } = await supabase
      .from("sporting_director_trophies")
      .select("id")
      .eq("sporting_director_id", currentProfile.id)
      .eq("trophy_key", EL_PRESIDENTE_TROPHY_KEY)
      .not("claimed_at", "is", null)
      .maybeSingle<{ id: string }>();

    if (presidentTrophyError || !presidentTrophy) {
      return {
        status: "error",
        message: "La tenue El Presidente n’est pas encore disponible.",
        fieldErrors: {
          avatarKey: [
            "Devenez président d’une fédération pour débloquer cette tenue.",
          ],
        },
      };
    }
  ...
  const selectedMedicalOutfit =
    selectedAvatar?.outfit === AMBULANCIER_AVATAR_OUTFIT_KEY
      ? {
          trophyKey: AMBULANCIER_TROPHY_KEY,
          rewardName: "Le chapeau d’infirmière",
          requirement: "Obtenez d’abord le trophée Ambulancier avec 5 blessés simultanés.",
        }
      : selectedAvatar?.outfit === EMERGENCY_DOCTOR_AVATAR_OUTFIT_KEY
        ? {
            trophyKey: EMERGENCY_DOCTOR_TROPHY_KEY,
            rewardName: "La tenue de Médecin urgentiste",
            requirement: "Obtenez d’abord le trophée Médecin urgentiste avec 10 blessés simultanés.",
          }
        : null;

  if (selectedMedicalOutfit) {
    const { data: medicalTrophy, error: medicalTrophyError } = await supabase
      .from("sporting_director_trophies")
      .select("id")
      .eq("sporting_director_id", currentProfile.id)
      .eq("trophy_key", selectedMedicalOutfit.trophyKey)
      .not("claimed_at", "is", null)
      .maybeSingle<{ id: string }>();

    if (medicalTrophyError || !medicalTrophy) {
      return {
        status: "error",
