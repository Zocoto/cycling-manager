...
export default async function NationalChampionshipResultsPage() {
  ...
  const [timeTrials, roadRaces] = await Promise.all([
    getCurrentTeamNationalChampionshipCountries({
      authUserId: user.id,
      calendar,
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-8 sm:py-12">
        ...
        <header className="mt-5 rounded-[2rem] bg-[linear-gradient(135deg,#071A17,#176951)] px-6 py-8 text-white shadow-[0_24px_70px_rgba(19,60,46,0.18)] sm:px-10 sm:py-10">
          ...
          <p className="mt-4 max-w-3xl text-sm font-semibold leading-6 text-[#D6DFD2] sm:text-base">
            Retrouvez les places de vos coureurs et ouvrez le classement
            officiel complet de chaque pays. Les résultats paraissent après la
            simulation de la discipline ; il n’y a pas de replay pour les CN.
          </p>
        </header>

        {timeTrials.length === 0 && roadRaces.length === 0 ? (
          <p className="mt-6 rounded-2xl border border-[#315B3E]/15 bg-white px-6 py-8 text-sm font-semibold text-[#60756E]">
            Aucun championnat national ne correspond actuellement aux
            nationalités des coureurs de votre effectif.
          </p>
        ) : (
