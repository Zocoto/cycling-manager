...
export async function completeCriteriumDiscoveryAction(): Promise<never> {
  const supabase =
    await createSupabaseServerClient();
  ...
  if (!progress || !run) {
    redirectWithError(
      "Inscrivez d’abord vos coureurs et ouvrez le replay du Critérium de la découverte.",
    );
  }
  ...
  if (
    ...
    if (
      ...
      redirectWithError(
        `La fin du Critérium n’a pas pu être validée : ${error?.message ?? "progression introuvable"}`,
      );
    }
  }
