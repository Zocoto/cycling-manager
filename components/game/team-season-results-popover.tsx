...
export function TeamSeasonResultsPopover({
  items,
  victoryCount,
}: {
  items: TeamSeasonResultPopoverItem[];
  victoryCount: number;
}) {
  const panelId = useId();
  ...
  return (
    <>
      <button
        ...
        className="group inline-flex min-h-10 items-center gap-2 rounded-full border border-[#278B70]/25 bg-[#E9F5F0] px-3.5 py-2 text-xs font-black text-[#176951] shadow-sm transition hover:-translate-y-0.5 hover:border-[#278B70]/45 hover:bg-[#DDF1E9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#278B70]"
      >
        <span
          aria-hidden="true"
        ...
        <span className="rounded-full bg-white/80 px-2 py-0.5 text-[10px] text-[#48665F]">
          {items.length}
        </span>
      </button>
      ...
      {open && typeof document !== "undefined"
        ? createPortal(
            <div
              ...
              id={panelId}
              role="dialog"
              aria-label="Résultats de la saison"
            ...
            >
              <div className="border-b border-white/10 bg-white/[0.045] px-4 py-3">
                ...
                <div className="mt-1 flex items-end justify-between gap-3">
                  <p className="font-black text-[#FFFDF4]">
                    Résultats de la saison
                  </p>
                  <span className="whitespace-nowrap text-[10px] font-bold text-[#9FB5A8]">
                    {victoryCount} victoire{victoryCount > 1 ? "s" : ""}
                  </span>
                </div>
              </div>
              ...
              <ul className="max-h-80 divide-y divide-white/10 overflow-y-auto">
                {items.map((item, index) => (
                  <li key={item.id}>
                    <Link
                      href={item.href}
                      className="group/result grid grid-cols-[auto_minmax(0,1fr)_auto] gap-3 px-4 py-3 transition hover:bg-white/[0.06] focus-visible:bg-white/[0.06] focus-visible:outline-none"
                    >
                      ...
                      <span
                        aria-hidden="true"
                        className="self-center text-[#7CCF9C] transition group-hover/result:translate-x-0.5"
                      >
                        →
                      </span>
                    </Link>
