    ...
    <section className="border-t-4 border-double border-[#241F18] px-5 py-6 sm:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button
          ...
          onClick={toggleLike}
          disabled={pending}
          className={`border px-4 py-2 text-xs font-black transition ${community.likedByViewer ? "border-[#A12742] bg-[#A12742] text-white" : "border-[#806C45]/50 bg-[#EFE4C8] hover:border-[#A12742]"}`}
        >
          {community.likedByViewer ? "J’aime cette édition" : "J’aime"} · {community.likeCount}
        </button>
      </div>
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <form onSubmit={submitComment} className="border border-[#806C45]/45 bg-[#EFE4C8]/60 p-4">
          ...
          <textarea id="cyclogazette-comment" value={message} onChange={(event) => setMessage(event.target.value)} maxLength={400} rows={3} placeholder="Votre regard sur cette journée de course…" className="mt-2 w-full resize-y border border-[#806C45]/45 bg-[#F8F0DB] p-3 font-serif text-sm outline-none focus:border-[#A12742]" />
        ...
        <div className="space-y-3">
          {community.comments.length > 0 ? community.comments.map((comment) => (
            <article key={comment.id} className="border-l-2 border-[#A12742] bg-[#EFE4C8]/45 px-4 py-3">
