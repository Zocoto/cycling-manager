...
export async function getYouthDevelopmentOverview(
  ...
  assertQuery(
    financeSettlement.error,
    "l’actualisation des finances du centre",
  );
...
async function loadOverview(admin: AdminClient, context: Context) {
  const [
    ...
    admin
      ...
      .select("country_id, facility_level")
      .returns<Array<{ country_id: string; facility_level: number }>>(),
    admin
  ...
  for (const [result, label] of [
    ...
    [facilitiesResult, "les installations"],
  ...
  const activeMissionByContract = new Map(
    (missionsResult.data ?? [])
      ...
      .map((mission) => [mission.scout_contract_id, mission.id]),
  );

  const countriesDto = await buildCountryDtos(
    admin,
    countries,
    facilitiesResult.data ?? [],
    context,
  );
  const countryDtoById = new Map(
    countriesDto.map((country) => [country.id, country]),
  );
  ...
  const candidateResult = missionIds.length
    ? await admin
        .from("youth_scouting_candidates")
        .select("*")
  ...
  const candidatesByMission = groupBy(
    candidateResult.data ?? [],
    (candidate) => candidate.mission_id,
  );
  const scoutByContractId = new Map(
    scouts.map((scout) => [scout.contractId, scout]),
  );
  ...
  const academy = academyRows.map((rider): AcademyYouth => {
    ...
    return {
      ...
      manualTraining: {
        ...
        currentSlotLabel:
          currentManualSlot === "manual_am" ? "Minuit – midi" : "Midi – minuit",
        currentSlotCompleted: Boolean(currentSlotSession),
        completedSlotCount: todaySessions.filter(
          (session) => session.training_mode === "manual",
        ).length,
...
async function buildCountryDtos(
  admin: AdminClient,
...
async function settleDueScoutingMissions(admin: AdminClient, context: Context) {
  ...
  const due = (result.data ?? []).filter(
    (mission) =>
      mission.season_id !== context.seasonId ||
      mission.completes_day_number <= context.currentDayNumber,
  );
...
async function completeMission(admin: AdminClient, mission: MissionRow) {
  const [
    ...
    admin
      ...
      .eq("id", mission.scout_contract_id)
      .single<{ staff_member_id: string }>(),
    admin
      .from("countries")
      .select("id, name, iso_alpha2, is_active")
    ...
    admin
      ...
      .maybeSingle<{ facility_level: number }>(),
    admin
      .from("country_rider_generation_profiles")
      .select("name_profile_code, avatar_profile_key")
      .eq("country_id", mission.country_id)
  ...
  const memberResult = await admin
    ...
    .eq("id", contract.staff_member_id)
    .single<{ country_id: string; level: number }>();
  assertQuery(memberResult.error, "le scout missionné");
  const scout = requireData(memberResult.data, "le scout missionné");
  const [talentsResult, teamSeasonResult, dailyRewardBoostResult] =
    await Promise.all([
      admin
  ...
  const insertion = await admin
    .from("youth_scouting_candidates")
    .upsert(candidates, {
      onConflict: "mission_id,report_slot",
...
async function scheduleTuition(
  ...
  const transactions = riders.flatMap((rider) =>
    [7, 14, 21, 28].flatMap((installmentDay, index) => {
      if (
        rider.joined_season_id === context.seasonId &&
        installmentDay < rider.joined_day_number
      )
        return [];
      ...
      return [
        {
          team_season_id: context.teamSeasonId,
          season_day_id: seasonDayId,
          day_number: installmentDay,
          amount:
            -Math.round((toNumber(rider.tuition_per_season) / 4) * 100) / 100,
          category: "training",
          status: "pending",
          description: `Frais de scolarité — ${rider.first_name} ${rider.last_name}`,
          source_reference: `youth-tuition:${rider.id}:${context.seasonId}:${index + 1}`,
        },
      ];
    }),
  );
  ...
  const insertion = await admin
    ...
    .upsert(transactions, {
      onConflict: "team_season_id,source_reference",
      ignoreDuplicates: true,
    });
...
async function settleAcademyTransitions(admin: AdminClient, context: Context) {
  ...
  for (const academy of result.data ?? []) {
    ...
    if (
      academy.status === "recruited" &&
      academy.promotion_game_year !== null &&
      context.gameYear >= academy.promotion_game_year
    ) {
      const riderId = await createPermanentRider(
        admin,
        academy,
        context,
        "active",
      ...
      const contract = await admin
        ...
        .insert({
          ...
          start_season_id: context.seasonId,
          end_season_id: context.seasonId,
          salary_per_season: 0,
          currency: context.currency,
          currency_code: context.currency,
          status: "active",
          signed_at: new Date().toISOString(),
          acquisition_type: "academy",
        });
      ...
      await createNotification(
        ...
        academy.team_id,
        "promoted",
        "Promotion en équipe première",
        `${academy.first_name} ${academy.last_name} rejoint officiellement l’effectif professionnel.`,
        `youth-promoted:${academy.id}`,
      );
      ...
      await createNotification(
        ...
        "contract_expired",
        "Fin de cursus jeune",
        `${academy.first_name} ${academy.last_name} a atteint 19 ans sans être recruté et rejoint les agents libres.`,
        `youth-expired:${academy.id}`,
      );
    }
  }
}

async function createPermanentRider(
  admin: AdminClient,
  academy: AcademyRow,
  context: Context,
  ...
  const rider = requireData(
    riderResult.data,
    "la création du coureur issu de l’école",
  );
  const ratings = ratingsToRow(scaleYouthRatings(rowToRatings(academy)));
  const seasonRatings = await admin
    .from("rider_season_ratings")
    .insert({
      rider_id: rider.id,
      season_id: context.seasonId,
      age: context.gameYear - academy.birth_game_year,
      ...ratings,
    });
...
async function createNotification(
  ...
  const result = await admin
    ...
    .upsert(
      ...
      { onConflict: "team_id,source_reference", ignoreDuplicates: true },
    );
...
async function loadContext(
  ...
  const [teamSeasonResult, welcomeCenterResult] = await Promise.all([
    admin
      .from("team_seasons")
      .select(
        "id, display_name, currency, cash_balance, registration_country_id",
      )
...
function rowToRatings(row: CandidateRow | AcademyRow): YouthRatings {
  return {
    ...
    hills: toNumber(row.hills),
    flat: toNumber(row.flat),
    timeTrial: toNumber(row.time_trial),
    cobbles: toNumber(row.cobbles),
    sprint: toNumber(row.sprint),
    acceleration: toNumber(row.acceleration),
    downhill: toNumber(row.downhill),
    endurance: toNumber(row.endurance),
    resistance: toNumber(row.resistance),
    recovery: toNumber(row.recovery),
    breakaway: toNumber(row.breakaway),
    prologue: toNumber(row.prologue),
  };
}
...
function scaleYouthRatings(ratings: YouthRatings): YouthRatings {
  return Object.fromEntries(
    YOUTH_RATING_KEYS.map((key) => [
      key,
      clamp(Math.round(34 + ratings[key] * 8), 0, 100),
    ]),
  ) as YouthRatings;
}
...
function requireData<T>(data: T | null, label: string): T {
  if (!data) throw new Error(`Impossible de charger ${label}.`);
  return data;
}
