...
function simulateIndividualTimeTrial(
  ...
  input.segments.forEach((segment, segmentIndex) => {
    for (const state of states.values()) {
      ...
      const effort = TIME_TRIAL_EFFORT_EFFECTS[
  ...
  const detached = [...breakaway]
    .sort(
      (first, second) =>
        ...
        (second.energy + getTerrainRating(second.rider, segment) * 0.45),
    )
    .slice(0, splitCount);
...
function validateTimeTrialPlans(input: StageSimulationInput) {
  ...
  if (
    ...
    throw new Error("Une préparation chrono ne peut viser qu’un contre-la-montre.");
  }
  ...
  for (const [riderId, plan] of Object.entries(plans)) {
    if (!ridersById.has(riderId) || !isTimeTrialEffortMode(plan.effortMode)) {
      throw new Error("Une consigne chrono vise un coureur ou un effort invalide.");
    }
    if (
