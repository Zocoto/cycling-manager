...
export function isRaceLeaderRole(role: RaceRole) {
  return role === "leader" || role === "leader_sprinter";
}
...
export type RaceVisualFrame = {
  ...
  /** Optional tactical data only produced by the dynamic road engine. */
  frontDynamics?: {
    breakawayCooperation: number;
...
export function getFinalBattleScenario(
  ...
  const lateJoiners = decisiveContenderIds
    ...
    .map((riderId) => {
      // Un coureur peut revenir d'un groupe typé `dropped` dans le dernier
      // tronçon. Ces groupes ne participent pas à la sélection des prétendants,
      // mais restent la source canonique de sa position à l'entrée du final.
      const origin = entrySnapshot.groups.find(
        (group) =>
          group.type !== "time_trial" && group.riderIds.includes(riderId),
      );

      return {
        riderId,
        fromGroupLabel: origin?.label ?? "Groupe intercalé",
        gapToLeaderSeconds: origin
          ? Math.max(0, origin.gapToLeaderSeconds - leadingGap)
          : 0,
      };
    });
...
export function assignAutomaticRaceRoles(
  ...
  for (const teamRiders of teams.values()) {
    ...
    const hasSprinterRole = () =>
      teamRiders.some((rider) => isRaceSprinterRole(rider.role));

    if (likelySprint && !hasSprinterRole()) {
...
function simulateRoadStage(input: StageSimulationInput): StageSimulationResult {
  ...
  const breakawayMomentumSeedRoll = random();
  let breakawayMomentum = clamp(
    breakawayChaseResistance + (breakawayMomentumSeedRoll - 0.5) * 0.3,
    0,
    1,
  );
  let breakawayGapSeconds = 0;
  let pursuitState = INITIAL_RACE_PURSUIT_STATE;
  let breakawayCooperationState: BreakawayCooperationState =
    INITIAL_BREAKAWAY_COOPERATION_STATE;
  let breakawayRelayTickIndex = 0;
  let lastAnnouncedCooperationBand: "organized" | "fractured" | null = null;
  ...
  input.segments.forEach((segment, segmentIndex) => {
    ...
    if (largeBreakawayDecision !== previousLargeBreakawayDecision) {
    ...
    const breakawayCooperationTicks: BreakawayCooperationState[] = [];
    ...
    if (breakaway.length > 0 && breakawayHasGivenUp) {
      ...
      breakawaySeconds *= 1 - momentumPaceBonus;
    }
    ...
    if (segmentIndex === 0) {
      ...
      let tickCompletedDistanceKm = completedDistanceKm;
      let dynamicBreakawaySeconds = 0;

      for (const [tickIndex, tick] of simulationTicks.entries()) {
        const tickShare = tick.distanceKm / segment.distanceKm;
        const tickRaceProgress = clamp(
          tickCompletedDistanceKm / totalDistanceKm,
          0,
          1,
        );
        ...
        breakawayCooperationState = evolveBreakawayCooperation({
          previousState: breakawayCooperationState,
          candidates: breakaway.map((state) => ({
            riderId: state.rider.id,
            teamId: state.rider.teamId,
            energy: state.energy,
            breakawayRating: state.rider.ratings.breakaway,
        ...
        breakawayCooperationTicks.push(breakawayCooperationState);
        breakawayRelayTickIndex += 1;
        breakawayMomentum = evolveBreakawayMomentum({
          ...
          pelotonAverageEnergy,
          chasePressure: pursuitState.pressure,
          cooperation: breakawayCooperationState.cooperation,
          selectiveTerrainShare,
          likelyMassSprint,
          // Seeded variability is initialized once for the stage. The live
          // balance then changes only because the situation changes.
          randomRoll: 0.5,
        });
        ...
        const naturalGap =
          ...
          breakawaySeconds *
            tickShare *
            breakawayCooperationState.paceTimeMultiplier;
        dynamicBreakawaySeconds +=
          breakawaySeconds *
          tickShare *
          breakawayCooperationState.paceTimeMultiplier;
        ...
        if (breakawayHasGivenUp) {
        ...
        } else if (tickRaceProgress < 0.3) {
          const allowedGap = Math.min(
            breakawayTargetGapSeconds * (1 - breakawayThreat * 0.28),
            95 + (segmentIndex + tickIndex / simulationTicks.length) * 58,
          );
          const convergenceShare = 1 - (1 - 0.48) ** tickShare;
          breakawayGapSeconds = clamp(
            Math.max(
              naturalGap,
              breakawayGapSeconds +
                Math.max(
                  8 * tickShare,
                  (allowedGap - breakawayGapSeconds) * convergenceShare,
                ),
            ),
            0,
            breakawayGapCeiling,
          ...
          const chaseClosingSeconds =
            (tickRaceProgress - 0.58) *
            (likelyMassSprint ? 115 : 72) *
            (1 - breakawayChaseResistance) *
            clamp(1 - breakawayMomentum * 0.82, 0.18, 1) *
            tickShare;
          breakawayGapSeconds = clamp(
    ...
    let cooperationCompletedDistanceKm = completedDistanceKm;
    while (breakawayCooperationTicks.length < simulationTicks.length) {
      const tickIndex = breakawayCooperationTicks.length;
      const tick = simulationTicks[tickIndex];
      if (breakaway.length === 0) {
        breakawayCooperationState = INITIAL_BREAKAWAY_COOPERATION_STATE;
        lastAnnouncedCooperationBand = null;
      } else {
        breakawayCooperationState = evolveBreakawayCooperation({
          previousState: breakawayCooperationState,
          candidates: breakaway.map((state) => ({
          ...
          tickIndex: breakawayRelayTickIndex,
          raceProgress: clamp(
            cooperationCompletedDistanceKm / totalDistanceKm,
            0,
            1,
          ),
    ...
    const finalCooperation = breakawayCooperationTicks.at(-1)?.cooperation;
    const cooperationBand =
      finalCooperation !== undefined && finalCooperation >= 0.69
        ? "organized"
        : finalCooperation !== undefined && finalCooperation <= 0.36
          ? "fractured"
          : null;
    if (
      breakaway.length > 1 &&
      cooperationBand &&
      cooperationBand !== lastAnnouncedCooperationBand &&
      commentary.length < 3
    ) {
      commentary.push(
        cooperationBand === "organized"
          ? "Les relais tournent bien dans l’échappée, chacun prend sa part de travail."
          : "L’entente se fissure à l’avant : plusieurs coureurs commencent à sauter des relais.",
      );
      lastAnnouncedCooperationBand = cooperationBand;
    }
      ...
      if (state.group === "breakaway") {
        ...
        const grupettoFactor =
          state.grupettoStatus === "active"
            ...
            : 1;
        const rawExtraLoss = getDroppedRiderLoss(
          state,
          segment,
          frontTerrainRating,
          input.profileType,
        ...
        const pursuit =
          ...
          (segment.terrain === "climb" || segment.surface === "cobbles")
            ...
            : { canPursue: false, lossMultiplier: 1, energyCost: 0 };
        const extraLoss =
          rawExtraLoss *
          survivalEscortFactor *
          grupettoFactor *
          pursuit.lossMultiplier;
        if (pursuit.canPursue) {
          state.energy = Math.max(0, state.energy - pursuit.energyCost);
          state.lateRecoveryEffortSeconds =
            (state.lateRecoveryEffortSeconds ?? 0) +
            pursuit.energyCost * 2.5;
        }
    ...
    const progressiveVisualFrames =
      segment.terrain === "climb" || segment.surface === "cobbles"
        ? spreadRoadGroupTransitionsAcrossFrames({
            frames: intermediateVisualFrames,
            finalSnapshot: officialSnapshot,
          })
        : intermediateVisualFrames;
    timeline.push(officialSnapshot);
    visualTimeline.push(
      ...progressiveVisualFrames.slice(0, -1),
      toRaceVisualFrame(
        officialSnapshot,
        segmentIndex,
        breakaway.length > 0
  ...
  const roadFinishMode = getRoadFinishMode(
    input.segments,
  ...
  if (finalOfficialSnapshot && visualTimeline.length > 0) {
    const finalFrontDynamics = visualTimeline.at(-1)?.frontDynamics;
...
export function selectStageAttackPlan(
  ...
  const maximum = getContextualBreakawayMaximum({
...
function getStageFavoriteRating(
  ...
  if (isLikelyMassSprint(segments)) {
    return (
      ...
      rider.form * 0.1 +
      (isRaceSprinterRole(rider.role) ? 4 : 0)
    );
  }
  ...
  return (
    ...
    rider.form * 0.05 +
    (isRaceLeaderRole(rider.role) ? 2 : 0)
  );
}
...
export function getDetachedRiderLatchOutcome({
  riderTerrainRating,
  targetPaceRating,
  resistanceRating,
  enduranceRating,
  ...
  terrain: RaceStageSegment["terrain"];
  surface: RaceStageSegment["surface"];
  isPriorityRider: boolean;
}) {
  const normalizedGapSeconds = Math.max(0, gapSeconds);
  const normalizedDifficulty = clamp(selectionDifficulty, 0, 1.4);
  const selectiveTerrain = terrain === "climb" || surface === "cobbles";
  const staminaRating = resistanceRating * 0.55 + enduranceRating * 0.45;
  const draftAllowance = Math.min(
    1.2,
    Math.log2(Math.max(1, targetGroupSize) + 1) *
      (terrain === "climb" ? 0.22 : 0.38),
  );
  const maximumLatchGapSeconds = selectiveTerrain
    ? clamp(
        5.5 +
          (isPriorityRider ? 2 : 0) +
          Math.max(0, energy - 30) * 0.035 +
          draftAllowance -
          normalizedDifficulty * 0.45,
        4,
        9,
      )
    : clamp(
  ...
  const minimumEnergy = selectiveTerrain
    ...
    : ABSOLUTE_EXHAUSTION_ENERGY + 1;
  const minimumForm = selectiveTerrain
    ? 35 + normalizedDifficulty * 8
    : 28;
  const maximumSustainableDeficit = selectiveTerrain
    ? clamp(
        5.7 +
          (isPriorityRider ? 1.4 : 0) +
          (staminaRating - 60) * 0.065 +
          Math.max(0, energy - 24) * 0.045 +
          draftAllowance -
          normalizedDifficulty * 0.8,
        4.5,
        9.5,
      )
    : clamp(
        8 +
          (isPriorityRider ? 1.2 : 0) +
          (staminaRating - 60) * 0.05 +
          draftAllowance,
        6,
        12,
      );
  const terrainDeficit = targetPaceRating - riderTerrainRating;
  const canLatch =
    normalizedGapSeconds <= maximumLatchGapSeconds &&
    energy >= minimumEnergy &&
    form >= minimumForm &&
    terrainDeficit <= maximumSustainableDeficit;
  const energyCost = canLatch
    ? clamp(
        0.45 +
          Math.max(0, normalizedGapSeconds - SAME_TIME_MAX_GAP_SECONDS) *
            (selectiveTerrain ? 0.24 : 0.15) +
          normalizedDifficulty * 0.8 +
          Math.max(0, terrainDeficit) * 0.08,
        0.45,
        3.8,
      )
    : 0;
  ...
  return {
    canLatch,
    energyCost,
    maximumLatchGapSeconds,
    maximumSustainableDeficit,
  };
}

export function getDetachedRiderPursuitOutcome({
  riderTerrainRating,
  fieldTerrainRating,
  resistanceRating,
  enduranceRating,
  ...
  const selectiveTerrain = terrain === "climb" || surface === "cobbles";
  const normalizedDifficulty = clamp(selectionDifficulty, 0, 1.4);
  const terrainDeficit = Math.max(0, fieldTerrainRating - riderTerrainRating);
  const staminaRating = resistanceRating * 0.55 + enduranceRating * 0.45;
  const energyFactor = clamp((energy - 16) / 42, 0, 1);
  const maximumPursuitDeficit = selectiveTerrain
    ? clamp(
        10 +
          (isPriorityRider ? 5 : 0) +
          (staminaRating - 60) * 0.08 +
          energyFactor * 2 -
          normalizedDifficulty * 0.7,
        7,
        19,
      )
    : 16;
  const canPursue =
    energy >= (selectiveTerrain ? 18 : 12) &&
    form >= (selectiveTerrain ? 45 : 35) &&
    terrainDeficit <= maximumPursuitDeficit;
  if (!canPursue) {
    return { canPursue: false, lossMultiplier: 1, energyCost: 0 };
  }

  const commitment = clamp(
    energyFactor * (isPriorityRider ? 1 : 0.78),
    0.18,
    1,
  );
  const lossMultiplier = selectiveTerrain
    ? clamp(
        0.86 -
          commitment * 0.32 +
          terrainDeficit * 0.0075 +
          normalizedDifficulty * 0.02,
        0.54,
        0.9,
      )
    : clamp(0.84 - commitment * 0.2, 0.58, 0.84);
  const energyCost = clamp(
    Math.max(0.5, segmentDistanceKm) *
      (selectiveTerrain
        ? 0.13 + normalizedDifficulty * 0.11
        : 0.055) *
      (0.72 + commitment * 0.38),
    0.45,
    3.6,
  );

  return { canPursue: true, lossMultiplier, energyCost };
}
...
function resolveSupportedLeaderRecovery({
  ...
  for (const leader of leaders) {
    ...
    const lateRaceEffortFactor = clamp((raceProgress - 0.7) / 0.3, 0, 1);
    ...
    for (const state of group) {
      ...
      state.lateRecoveryEffortSeconds =
        (state.lateRecoveryEffortSeconds ?? 0) +
        recoveredSeconds * lateRaceEffortFactor;
    }
...
function dropStrugglingRiders({
  ...
  for (const state of peloton) {
    ...
    const wheelFollowingOutcome = getDetachedRiderLatchOutcome({
      riderTerrainRating: getStateSelectionTerrainRating(
        state,
        segment,
        profileType,
      ...
      targetPaceRating: frontTerrainRating,
      resistanceRating: state.rider.ratings.resistance,
      enduranceRating: state.rider.ratings.endurance,
      energy: state.energy,
      form: state.rider.form,
      gapSeconds: Math.max(
        0,
        Math.min(5, effectiveDeficit - ruptureThreshold + 1),
      ),
      targetGroupSize: peloton.length,
      selectionDifficulty,
      terrain: segment.terrain,
      surface: segment.surface,
      isPriorityRider:
    ...
    const spendsReserveToHoldWheels =
      !losesContactFromExhaustion &&
      state.energy >= 24 &&
      effectiveDeficit > ruptureThreshold &&
      wheelFollowingOutcome.canLatch;

    if (
      state.energy < ABSOLUTE_EXHAUSTION_ENERGY ||
      losesContactFromExhaustion ||
      (effectiveDeficit > ruptureThreshold &&
        !exceptionallyHoldsOn &&
        !spendsReserveToHoldWheels)
    ) {
      const immediateLossSeconds =
        5 +
    ...
    } else if (spendsReserveToHoldWheels) {
      state.energy = Math.max(
        0,
        state.energy - wheelFollowingOutcome.energyCost,
      );
      state.lateRecoveryEffortSeconds =
        (state.lateRecoveryEffortSeconds ?? 0) +
        wheelFollowingOutcome.energyCost * 2;
      if (commentary.length < 4) {
        commentary.push(
          `${state.rider.name} puise dans ses réserves pour accrocher les roues et rester dans le groupe.`,
        );
      }
...
function resolveNearbyDetachedGroupLatching({
  states,
  segment,
  segmentIndex,
  profileType,
  ...
  const pelotonTime = peloton.length
    ? average(peloton.map((state) => state.elapsedTimeSeconds))
    : null;
  const latchableDetachedStates = [...states.values()].filter(
    (state) =>
      (state.group === "delayed" || state.group === "dropped") &&
      state.leaderRecoveryStatus === undefined &&
  ...
  for (let iteration = 0; iteration < maximumIterations; iteration += 1) {
    ...
    const detachedGroups = splitElapsedRiderGroups(
    ...
    const orderedGroups = [
      ...(currentPeloton.length > 0 ? [currentPeloton] : []),
      ...detachedGroups,
    ].sort(
      (first, second) =>
        average(first.map((state) => state.elapsedTimeSeconds)) -
    ...
    for (let groupIndex = 1; groupIndex < orderedGroups.length; groupIndex += 1) {
      const frontGroup = orderedGroups[groupIndex - 1];
      const rearGroup = orderedGroups[groupIndex];
      const frontTime = average(
        frontGroup.map((state) => state.elapsedTimeSeconds),
      );
      const rearTime = average(
        rearGroup.map((state) => state.elapsedTimeSeconds),
      );
      const gapSeconds = Math.max(0, rearTime - frontTime);
      const targetPaceRating = getFrontTerrainRating(
        frontGroup,
        segment,
        profileType,
        hillyClimbLoad,
      ...
      const outcomes = new Map(
        rearGroup.map((state) => [
          ...
          getDetachedRiderLatchOutcome({
            ...
            form: state.rider.form,
            gapSeconds,
            targetGroupSize: frontGroup.length,
            selectionDifficulty,
            terrain: segment.terrain,
            surface: segment.surface,
            isPriorityRider:
      ...
      const physicallyAbleIds = new Set(
        rearGroup
          .filter((state) => outcomes.get(state.rider.id)?.canLatch)
          .map((state) => state.rider.id),
      );
      const frontRiderIds = new Set(
        frontGroup.map((state) => state.rider.id),
      );
      const followers = rearGroup.filter(
        (state) =>
          physicallyAbleIds.has(state.rider.id) &&
          (!state.supportingLeaderId ||
            physicallyAbleIds.has(state.supportingLeaderId) ||
            frontRiderIds.has(state.supportingLeaderId)),
      );
      if (followers.length === 0) continue;

      const targetGroup = frontGroup.some((state) => state.group === "peloton")
        ? "peloton"
        : frontGroup.some((state) => state.group === "delayed")
          ? "delayed"
          : "dropped";
      for (const state of followers) {
        if (state.leaderRecoveryStatus !== undefined) {
          clearLeaderRecoveryAssignment(state, states);
        }
      }
      for (const state of followers) {
        const outcome = outcomes.get(state.rider.id)!;
        state.group = targetGroup;
        state.groupSinceSegment = segmentIndex;
        state.elapsedTimeSeconds = frontTime;
        state.lostTimeSeconds =
          targetGroup === "peloton" || pelotonTime === null
            ? 0
            : Math.max(0, frontTime - pelotonTime);
        state.energy = Math.max(0, state.energy - outcome.energyCost);
        state.lateRecoveryEffortSeconds =
          (state.lateRecoveryEffortSeconds ?? 0) +
          Math.max(1, gapSeconds) *
            (segment.terrain === "climb" ? 0.7 : 0.45);
        delete state.supportingLeaderId;
        delete state.grupettoStatus;
        delete state.grupettoPacePressure;
      }
      latchedStates.push(...followers);
  ...
  if (latchedStates.length > 0 && commentary.length < 4) {
    const uniqueLatchedStates = [
      ...new Map(
        latchedStates.map((state) => [state.rider.id, state]),
      ).values(),
    ];
    commentary.push(
      `${formatRiderList(uniqueLatchedStates)} ${uniqueLatchedStates.length > 1 ? "font l’effort pour prendre les roues du groupe qui les précède" : "fait l’effort pour prendre les roues du groupe qui le précède"}.`,
    );
  }
}
...
function getPrimeScore(
  ...
  const roleBonus =
    prime.type === "mountain" && state.rider.role === "mountain_classification"
      ? 16
      : prime.type === "intermediate_sprint" &&
          isRaceSprinterRole(state.rider.role)
        ? 10
        : isRaceLeaderRole(state.rider.role)
          ? -4
...
function getRoadFinishScores(
  ...
  const massSprintFinish = finishMode === "mass_sprint";
  const reducedSprintFinish = finishMode === "reduced_sprint";
  const compactSprintFinish = massSprintFinish || reducedSprintFinish;
  const peloton = getStatesInGroup(states, "peloton");
  ...
  const sprintContenders = [...peloton]
    .filter((state) => isRaceSprinterRole(state.rider.role))
    .sort(
      (first, second) =>
  ...
  for (const state of states.values()) {
    ...
    if (longSummitFinishFactor > 0) {
    ...
    } else if (massSprintFinish && state.group !== "abandoned") {
      ...
      const pistardPositioningBonus = hasSpecialAbility(rider, "pistard")
        ? 2
        : 0;
      const roleFactor = isRaceSprinterRole(rider.role)
        ? 3
        : rider.role === "leadout"
          ? -3
          : 0;
      ...
      const positioningBonus =
        trainRank === 0 ? 2 : trainRank === 1 ? 1.2 : trainRank === 2 ? 0.5 : 0;
      const roleBonus = isRaceSprinterRole(rider.role)
        ? 1.5
        : isRaceLeaderRole(rider.role)
          ? 2
          : rider.role === "leadout"
      ...
      const roleBonus = isRaceLeaderRole(rider.role) ? 3.5 : 0;
    ...
    scores.set(
      ...
      score -
        ...
        getCollectiveWorkFinishPenalty({
          ...
          states,
          segments,
          sprintFinish: compactSprintFinish,
        }) +
        random() *
          SCORE_NOISE *
          scoreNoiseFactor *
        ...
        Math.min(6, (state.lateRecoveryEffortSeconds ?? 0) * 0.18) -
        state.finalSprintCrashFinishPenalty,
    );
  }
...
export function getReducedSprintFinishBaseScore({
  ...
  return (
    rider.ratings.sprint * 0.36 +
    rider.ratings.acceleration * 0.14 +
    decisiveFinishRating * 0.22 +
...
function toRaceVisualFrame(
  ...
  const visibleBreakawayRiderIds = new Set(
    snapshot.groups
      .filter((group) => group.type === "breakaway")
      .flatMap((group) => group.riderIds),
  );
  const normalizedFrontDynamics = frontDynamics
    ? {
        ...frontDynamics,
        activeRelayRiderIds: frontDynamics.activeRelayRiderIds.filter(
          (riderId) => visibleBreakawayRiderIds.has(riderId),
        ),
      }
    : undefined;
  ...
  return {
    ...
    sourceTimelineIndex,
    ...(normalizedFrontDynamics
      ? { frontDynamics: normalizedFrontDynamics }
      : {}),
  };
}

export function spreadRoadGroupTransitionsAcrossFrames({
  frames,
  finalSnapshot,
}: {
  frames: RaceVisualFrame[];
  finalSnapshot: RaceTimelineSnapshot;
}) {
  if (frames.length < 3 || finalSnapshot.groups.length === 0) return frames;
  const referenceFrame = frames.at(-1)!;
  const referenceGroupByRiderId = new Map(
    referenceFrame.groups.flatMap((group) =>
      group.riderIds.map((riderId) => [riderId, group] as const),
    ),
  );
  const finalGroupByRiderId = new Map(
    finalSnapshot.groups.flatMap((group) =>
      group.riderIds.map((riderId) => [riderId, group] as const),
    ),
  );
  const hasGroupTransition = [...finalGroupByRiderId].some(
    ([riderId, finalGroup]) =>
      referenceGroupByRiderId.get(riderId)?.id !== finalGroup.id,
  );
  if (!hasGroupTransition) return frames;

  const transitionFrameCount = Math.min(
    4,
    Math.max(2, Math.ceil(frames.length * 0.35)),
  );
  const transitionStartIndex = Math.max(
    0,
    frames.length - transitionFrameCount,
  );

  return frames.map((frame, frameIndex) => {
    if (frameIndex < transitionStartIndex) return frame;
    const transitionProgress =
      (frameIndex - transitionStartIndex + 1) /
      (frames.length - transitionStartIndex + 1);
    const groups = finalSnapshot.groups.map((finalGroup) => {
      const sourceGroups = finalGroup.riderIds
        .map((riderId) => referenceGroupByRiderId.get(riderId))
        .filter((group): group is RaceGroupSnapshot => Boolean(group));
      const sourceGap =
        sourceGroups.length > 0
          ? average(sourceGroups.map((group) => group.gapToLeaderSeconds))
          : finalGroup.gapToLeaderSeconds;
      const sourceEnergy =
        sourceGroups.length > 0
          ? average(sourceGroups.map((group) => group.averageEnergy))
          : finalGroup.averageEnergy;

      return {
        ...finalGroup,
        gapToLeaderSeconds: Math.round(
          sourceGap +
            (finalGroup.gapToLeaderSeconds - sourceGap) * transitionProgress,
        ),
        averageEnergy: round(
          sourceEnergy +
            (finalGroup.averageEnergy - sourceEnergy) * transitionProgress,
          1,
        ),
      };
    });
      ...
      ...(frame.frontDynamics
        ? {
            frontDynamics: {
              ...frame.frontDynamics,
              activeRelayRiderIds:
                frame.frontDynamics.activeRelayRiderIds.filter((riderId) =>
                  visibleBreakawayRiderIds.has(riderId),
                ),
            },
          }
        : {}),
...
/**
 * Un final roulant ne suffit pas à faire disparaître la sélection produite par
 * une difficulté proche de l'arrivée. En particulier, une côte raide placée
 * avant le dernier plat conduit à un sprint réduit, pas à un sprint massif.
 */
export function isLikelyMassSprint(segments: RaceStageSegment[]) {
  const finalSegment = segments.at(-1);
  if (
    !finalSegment ||
    finalSegment.terrain !== "flat" ||
  ...
  let distanceAfterSegmentKm = 0;
  const decisiveFinishWindowKm = 35;
  for (let index = segments.length - 1; index >= 0; index -= 1) {
    const segment = segments[index];
    if (distanceAfterSegmentKm >= decisiveFinishWindowKm) break;
    const distanceInsideWindowKm = Math.min(
      segment.distanceKm,
      decisiveFinishWindowKm - distanceAfterSegmentKm,
    );
    const gradient = Math.max(0, segment.averageGradientPct);
    const climbLoad = distanceInsideWindowKm * gradient;
    const decisiveClimb =
      segment.terrain === "climb" &&
      distanceInsideWindowKm >= 3 &&
      (gradient >= 8 ||
        climbLoad >= 28 ||
        (distanceInsideWindowKm >= 6 && gradient >= 5.5));
    const decisiveCobbles =
      segment.surface === "cobbles" && distanceInsideWindowKm >= 4;
    if (decisiveClimb || decisiveCobbles) return false;

    distanceAfterSegmentKm += segment.distanceKm;
  }
...
export function getRoadFinishMode(
  segments: RaceStageSegment[],
  leadingGroupSize: number,
): RoadFinishMode {
  if (isLikelyMassSprint(segments)) return "mass_sprint";

  return isFlatRunInGroupSprint(segments, leadingGroupSize)
    ? "reduced_sprint"
    : "selective";
}
...
function validateExplicitRoles(riders: RiderSimulationInput[]) {
  ...
  for (const teamRiders of teams.values()) {
    if (teamRiders.filter((rider) => rider.role === "leader").length > 1) {
      throw new Error("Une équipe ne peut désigner qu’un seul leader.");
    }
    if (
      teamRiders.filter((rider) => isRaceSprinterRole(rider.role)).length > 1
    ) {
      throw new Error(
