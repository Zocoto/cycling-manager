...
describe("questions après-course", () => {
  it("conserve une bibliothèque éclectique de trente questions", () => {
    expect(POST_RACE_INTERVIEW_QUESTION_POOL).toHaveLength(30);
    expect(new Set(POST_RACE_INTERVIEW_QUESTION_POOL.map(({ id }) => id)).size).toBe(30);
  });
