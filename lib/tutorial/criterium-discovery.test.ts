...
describe("Critérium de la découverte", () => {
  ...
  it("offre une première victoire sans modifier les données sportives du joueur", () => {
    const playerRiders = Array.from(
      ...
      (_, index) =>
        createTutorialPlayerRider(index),
    );

    const initialPlayerSnapshot =
      structuredClone(playerRiders);

    const roster = playerRiders.map(
      (rider, index) => ({
        riderId: rider.id,
        role:
          index === 0
    ...
    const opponents =
      run.lockedSimulation.input.riders.filter(
        (rider) =>
          !playerIds.has(rider.id),
      );

    const winner =
      run.lockedSimulation.simulation.results.find(
        (result) => result.rank === 1,
      );

    expect(opponents).toHaveLength(
      CRITERIUM_DISCOVERY_OPPONENT_COUNT,
    );
    expect(
      opponents.every(
    ...
    expect(
      winner &&
        playerIds.has(winner.riderId),
    ).toBe(true);
    expect(playerRiders).toEqual(
...
function createTutorialPlayerRider(
  index: number,
